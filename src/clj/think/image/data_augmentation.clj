...
(defn- interp->affine-op
  ^long [interp]
  (condp = interp
    :bilinear AffineTransformOp/TYPE_BILINEAR
    :nearest-neighbor AffineTransformOp/TYPE_NEAREST_NEIGHBOR
...
(defn affine-scale
  ...
  (let [tx (AffineTransform.)
        ...
        src-height (think-image/height src)
        out-image (i/new-image (long (* src-width (double scale-x)))
                               (long (* src-height (double scale-y))))
        _ (.scale tx scale-x scale-y)
        op (AffineTransformOp. tx (interp->affine-op interpolation))]
    (.filter op src out-image)
    out-image))
